...
float infertime = 0, updatetime = 0;
int effectfrontier = 0;
...
void infertimeHandler(const std_msgs::msg::Float32::ConstSharedPtr infertimeIn)
{
  infertime = infertimeIn->data;
  fprintf(perfFilePtr, "%f %f %d\n", infertime, updatetime, effectfrontier);
  fflush(perfFilePtr);  
}

void updatetimeHandler(const std_msgs::msg::Float32::ConstSharedPtr updatetimeIn)
{
  updatetime = updatetimeIn->data;
}

void frontierHandler(const std_msgs::msg::Int32::ConstSharedPtr frontierIn)
{
  effectfrontier = frontierIn->data;
}
...
int main(int argc, char** argv)
{
  ...
  // new codes
  // std::string ns_prefix = nh->get_namespace();
  // if (ns_prefix == "/"){
  //   ns_prefix.clear();
  // }
  // if (!ns_prefix.empty() && ns_prefix.front() == '/')
  // {
  //   ns_prefix.erase(0, 1);   // delete '/' of namespace string
  // }
  // if (!ns_prefix.empty())
  // {
  //   ns_prefix += "/";        // add '_'
  ...
  nh->declare_parameter<std::string>("perfFile", perfFile); 
  ...
  nh->get_parameter("perfFile", perfFile);
  ...
  perfFile.replace(perfFile.find("/install/"), 8, "/src");
  ...
  auto subInfertime = nh->create_subscription<std_msgs::msg::Float32>("inference_time", 5, infertimeHandler);

  auto subUpdatetime = nh->create_subscription<std_msgs::msg::Float32>("update_graph_time", 5, updatetimeHandler);

  auto subFrontiers = nh->create_subscription<std_msgs::msg::Int32>("effect_frontier", 5, frontierHandler);
  ...
  if (ply_reader.read(mapFile, *overallMapCloud) == -1) {
    ...
    RCLCPP_INFO(nh->get_logger(),
                "Loaded map cloud: %zu points",
                overallMapCloud->points.size());
  }
  ...
  RCLCPP_INFO(nh->get_logger(),
              "Map cloud after voxel filter: %zu points",
  ...
  metricFile += "_" + robot_id + "_" + timeString + ".txt";
  trajFile += "_" + robot_id + "_" + ".txt";
  perfFile += "_" + robot_id + "_" + ".txt";
  metricFilePtr = fopen(metricFile.c_str(), "w");
  trajFilePtr = fopen(trajFile.c_str(), "w");
  perfFilePtr = fopen(perfFile.c_str(), "w");

  fprintf(metricFilePtr, "# exploredVolume travelingDis runtime timeDuration\n");
  fprintf(trajFilePtr, "# x y z roll pitch yaw timeDuration\n");
  fprintf(perfFilePtr, "# plannTime buildGraphTime frontiers\n");
  fflush(perfFilePtr); 

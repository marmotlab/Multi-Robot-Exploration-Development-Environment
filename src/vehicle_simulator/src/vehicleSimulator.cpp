...
int main(int argc, char** argv)
{
  ...
  //   ns_prefix += "/";        // add '_'
  // }
  nh->declare_parameter<string>("namespace", ns_prefix);
  nh->get_parameter("namespace", ns_prefix);
  ns_prefix += "/";
  std::cout << "!!! ns_prefix: " << ns_prefix << std::endl;
  ...
  // use lambda to bring the ns_prefix in the handler
  ...
  odomData.child_frame_id = ns_prefix + "sensor";
  ...
  cameraState.name = ns_prefix + "camera";
  gazebo_msgs::msg::EntityState lidarState;
  lidarState.name = ns_prefix + "lidar";
  gazebo_msgs::msg::EntityState robotState;
  robotState.name = ns_prefix + "robot";

  std::cout << "~~~robotState.name: " << robotState.name << std::endl;

  // modify the srv name from /set_entility_state to /gazebo/set_entility_state
  ...
  //   RCLCPP_ERROR(nh->get_logger(), "Service /set_entity_state not available after waiting");
  //   return 1;
  // }
  ...
  while (status)
  {
    ...
    transformTfGeom.child_frame_id = ns_prefix + "sensor";
    ...
    // std::cout << "~~~request->state.name: " << request->state.name << std::endl;
    // std::cout << "vehicleX: " << vehicleX << std::endl;
